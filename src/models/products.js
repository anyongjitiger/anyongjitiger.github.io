...
import { clone, each, extend, keys, matches, sortedIndex, values } from 'underscore';
...
var catalogs = {
  ...
  oscar: utils.loadJson([OSCAR_PATH, 'catalog.json'].join('/'))
};
  ...
  return extend(
    {
      ...
      load: function (cancel) {
        var me = this;
        return Promise.all(this.paths, utils.loadJson).then(function (files) {
          return cancel.requested ? null : extend(me, buildGrid(me.builder.apply(me, files)));
        });
      }
    },
    overrides
...
function gfs1p0degPath(attr, type, surface, level) {
  ...
  var file = [stamp, type, surface, level, 'gfs', '1.0'].filter(utils.isValue).join('-') + '.json';
...
function describeSurface(attr) {
  return attr.surface === 'surface' ? 'Surface' : utils.capitalize(attr.level);
}

function describeSurfaceJa(attr) {
  return attr.surface === 'surface' ? '地上' : utils.capitalize(attr.level);
}
...
var FACTORIES = {
    ...
    create: function (attr) {
      ...
      return buildProduct({
        ...
        builder: function (windFile, airdensFile) {
          ...
          return {
            header: clone(airdensBuilder.header),
  ...
  currents: {
    ...
    create: function (attr) {
      return catalogs.oscar.then(function (catalog) {
        return buildProduct({
          field: 'vector',
          type: 'currents',
          description: localize({
          ...
          builder: function (file) {
            ...
            return {
              ...
              data: function (i) {
                var u = uData[i],
                  v = vData[i];
                return utils.isValue(u) && utils.isValue(v) ? [u, v] : null;
              }
            };
          },
          units: [
...
function lookupOscar(catalog, date, offset) {
  ...
  var prefix = utils.ymdRedelimit(date, '/', ''),
    i = sortedIndex(catalog, prefix);
...
function oscarDate(catalog, attr) {
  var file = lookupOscar(catalog, attr.date);
  var parts = file ? utils.ymdRedelimit(file, '', '/').split('/') : null;
...
function buildGrid(builder) {
  ...
  function interpolate(λ, φ) {
    var i = utils.floorMod(λ - λ0, 360) / Δλ; // calculate longitude index in wrapped range [0, 360)
...
export function productsFor(attributes) {
  var attr = clone(attributes),
    results = [];
  values(FACTORIES).forEach(function (factory) {
    if (factory.matches(attr)) {
      results.push(factory.create(attr));
    }
  });
  return results.filter(utils.isValue);
}

export const overlayTypes = new Set(keys(FACTORIES));
